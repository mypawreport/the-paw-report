...
- **v143** — Brought back the edge-lighting color shimmer (removed in the v140 perf fix) using a lightweight method — the hue animation now runs on just the 4 strip containers instead of on every individual icon, so it doesn't reintroduce the lag that v140 fixed.
